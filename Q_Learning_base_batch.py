...
model.load_weights("weights_batch")
f = open("scorelogs.txt", 'w')
f1 = open("Q_function_advancement", 'w')
...
for k in range(it+1):
    ...
    if global_reward==-1:
        ...
        for k in range(1,len(states)):
            ...
            states_batch += [[states[len(states)-k-1]]]
            rewards_batch += [new_reward]

            #model.fit([[states[len(states)-k-1]]],[new_reward],batch_size=1,epochs=1)

        model.fit(states_batch,rewards_batch,batch_size=len(states),epochs=1)
    ...
    if not(k%100):
        model.save_weights("weights_batch")
